...
export async function syncFromSupabase() {
  ...
  try {
    ...
    state = {
      ...state,
      cheios: cheiosRes.data ? cheiosRes.data.map((c: any) => ({
      ...
      imports: importsRes.data ? importsRes.data.map((i: any) => ({
        id: i.id,
      ...
      priorityRequests: prioritiesRes.data ? prioritiesRes.data.map((p: any) => ({
        id: p.id,
...
export function addPriorityRequest(req: import("./types").PriorityRequest) {
  ...
  supabase.from("priority_requests").insert({
    id: req.id,
  ...
  }).then(({ error }) => { if (error) toast.error("Erro ao salvar prioridade."); });
  emit();
}
...
export function updatePriorityStatus(id: string, status: import("./types").RequestStatus) {
  ...
  supabase.from("priority_requests").update({ status }).eq("id", id).then(({ error }) => { if (error) toast.error("Erro ao atualizar status."); });
  emit();
}

export function deletePriorityRequest(id: string) {
  state = { ...state, priorityRequests: state.priorityRequests.filter((r) => r.id !== id) };
  supabase.from("priority_requests").delete().eq("id", id).then(({ error }) => { if (error) toast.error("Erro ao remover prioridade."); });
  emit();
}